...
fn main() {
    ...
    //
    bunny_nibbles::<Carrot>(&mut carrot, 10);
    println!("Bunny nibbles for awhile: {:?}", carrot);

    bunny_nibbles::<Grapes>(&mut grapes, 20);
...
fn bunny_nibbles<T: Bite>(biter : &mut dyn Bite, times_to_bite: i32) {
    for i in 0..times_to_bite {
        biter.bite()
    }
}